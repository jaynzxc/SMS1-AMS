    ...
    if (format === 'CSV') {
      ...
      if (table) {
        ...
        visibleRows.forEach(row => {
          ...
          const date = cells[2]?.querySelector('div:first-of-type')?.textContent.trim() || '';
          ...
          link.setAttribute("download", `Scan_Logs_${specificDate}.csv`);
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
      }
    }
    showToast('Download Ready', `Scan logs report exported successfully (${format})`, 'success');
  }, 800);
}
